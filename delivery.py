...
class TwitterDelivery(Delivery):
    """Posts tweet-sized messages to Twitter/X via Playwright.
    ...
    def send(self, message):
        if len(message) > self.MAX_CHARS:
            log(f"  ℹ Twitter: skipping ({len(message)} chars > {self.MAX_CHARS})")
            return False

        try:
            from playwright.sync_api import sync_playwright, TimeoutError as PWTimeout
        except ImportError:
            log("  ❌ Twitter: playwright not installed.")
        ...
        with sync_playwright() as p:
            ...
            if os.path.isfile(self.session_file):
                try:
                    ...
                    log("  ℹ Twitter: loaded saved session")
                except Exception:
                    pass
            ...
            try:
                ...
                log("  ❌ Twitter: timed out waiting for page element")
                browser.close()
                return False
            except Exception as e:
                log(f"  ❌ Twitter: {e}")
    ...
    def _login(self, page):
        ...
        try:
            ...
            if "login" in page.url:
                log("  ❌ Twitter: login failed — check TWITTER_USERNAME / TWITTER_PASSWORD in .env")
                return False

            log("  ✅ Twitter: logged in")
            return True
        except Exception as e:
            log(f"  ❌ Twitter: login error: {e}")
            return False
...
REGISTRY = {
    ...
    "twitter": lambda cfg: TwitterDelivery(
